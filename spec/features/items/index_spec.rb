...
RSpec.describe 'Items Index' do
  ...
  describe 'As a visitor' do
    describe 'when i click a link to view all of the items that a supermarket has' do
      it 'I am taken to the supermarket item index page and I can see a unique list of all the items that the supermarket has' do

        visit "/supermarkets/#{@traderjoes.id}"

        click_link "List of Trader Joes Items"
        
        expect(current_path).to eq("/supermarkets/#{@traderjoes.id}/items")

        within "#item-#{@broccoli.id}" do
          expect(page).to have_content(@broccoli.name)
        end

        within "#item-#{@potato.id}" do
          expect(page).to have_content(@potato.name)
        end

        within "#item-#{@popcorn.id}" do
          expect(page).to have_content(@popcorn.name)
        end
